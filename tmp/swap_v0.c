...
Transaction convertToTransaction(const PayContract* payContract) {
    ...
    transaction.source = payContract->payee;
    transaction.destination = payContract->receiver;
    transaction.amount = payContract->amount;
    transaction.currency = payContract->currency;
    return transaction;
}
...
PayContract* parsePayPayload(char* parameters) {
    ...
    payContract->payee = token;

    token = strtok(NULL, " ");
    payContract->receiver = token;
    
    token = strtok(NULL, " ");
    payContract->amount = atoi(token);
    
    token = strtok(NULL, " ");
    payContract->currency = token;
...
DepositContract parseDepositPayload(char* parameters) {
    ...
    // Split the parameters string into tokens
    char* token = strtok(parameters, " ");
    depositContract.depositor = token;

    token = strtok(NULL, " ");
    depositContract.receiver = token;

    token = strtok(NULL, " ");
    depositContract.amount = atoi(token);

    token = strtok(NULL, " ");
    depositContract.currency = token;
...
Contract* newContract(ContractType type, const char* parameters, Contract* subContractOK, Contract* continueAs) {
    ...
    if (parameters != NULL) {
        size_t len = strlen(parameters) + 1;
        contract->parameters = malloc(len);
        memcpy(contract->parameters, parameters, len);
    } else {
...
int main() {
    ...
    Contract* whenDepositAdaProvider = newContract(DEPOSIT, "AdaProvider AdaProvider 50 ADA 10", whenDepositDollarProvider, FailedContract);
